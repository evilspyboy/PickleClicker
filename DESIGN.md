...
- **Tax Audits (The Popup Event):** The Tax Office constantly watches the player's wealth. Audits are not strictly on a set timer; they occur based on an **Audit Exposure** percentage.
  ...
  - **The Audit Process & Penalties:** When an audit triggers, a popup modal (reusing the pastel `#custom-modal` style from Game 1) pauses the game. The Tax Office attempts to seize wealth, but they never wipe the player out entirely (a maximum cap applies to all penalties).
    ...
    - *Seizure:* If the investigation succeeds, the Tax Office seizes a portion of the player's Shell Companies (if more than 1 are owned). More shell companies allow more money to be hidden from the *liquid* penalty, but carry a much higher risk of total loss during the *seizure* penalty. If no seizable shell companies exist, they may seize a Legit Business (if more than 5 of a specific type are owned). Tangible Assets are explicitly safe from seizure.
...
- **Businesses & Assets:** Players use liquid biscuits to buy Tangible Assets (e.g., Cat Yacht, Solid Gold Litterbox) and Businesses. Both can be bought and sold.
  - **Tangible Assets (Hiding Wealth):** Act as safe havens to hide biscuits from standard liquid biscuit audits, and are completely immune to seizure during an investigation. They have no direct effect on the market. Their buy/sell price remains fixed, providing a stable and reliable vault for wealth. The game will feature scaling assets with massive price tags (e.g., Solid Gold Litterbox vs. Private Island Litterbox).
